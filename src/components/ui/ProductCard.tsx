    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 bottom-0 overflow-hidden rounded-[28px] transition-all duration-500 ease-out group-hover:top-2 group-hover:left-2 group-hover:right-2 group-hover:bottom-43 group-hover:rounded-2xl">
        ...
        <div className="absolute top-3 left-3 z-10 flex flex-col gap-1">
          {discount && (
            <span className="bg-[#E8500A] text-white text-[11px] font-medium px-2.5 py-0.75 rounded-full">
              -{discount}%
            </span>
          )}
          {product.isNew && !discount && (
            <span className="bg-border-mint text-[#1A6B3C] text-[11px] font-medium px-2.5 py-0.75 rounded-full">
              NEW
            </span>
          )}
          {product.isFlashDeal && !discount && !product.isNew && (
            <span className="bg-ember-surface text-[#B84000] text-[11px] font-medium px-2.5 py-0.75 rounded-full">
              DEAL
            </span>
          )}
        </div>
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 flex h-43 flex-col justify-center gap-1 px-4 py-3">
        <p className="text-[11px] font-medium text-white/70 group-hover:text-gray-500 transition-colors duration-300">
          {product.brand}
        </p>
        <h3 className="text-org-sm font-medium text-white group-hover:text-gray-900 line-clamp-2 leading-[1.4] transition-colors duration-300">
          {product.name}
        </h3>
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-[11px] text-white/70 group-hover:text-gray-400 transition-colors duration-300">
            ({product.reviewCount})
          </span>
        </div>
        ...
        <div className="flex items-baseline gap-2">
          <span className="font-mono text-org-sm font-semibold text-white group-hover:text-[#25A55A] transition-colors duration-300">
        ...
        {product.status !== "out_of_stock" && (
          <button
            ...
            className={`mt-1 w-full py-2.25 rounded-full text-org-sm font-medium transition-all flex items-center justify-center gap-1.5 ${
              addedToCart
                ? "bg-border-mint text-[#1A6B3C]"
                : "bg-white/90 text-gray-900 group-hover:bg-gray-900 group-hover:text-white"
            }`}
          >
