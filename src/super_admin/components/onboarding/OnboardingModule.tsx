...
const BLANK_FORM: OnboardingForm = {
  ...
  shopName: "", shopTagline: "", shopLogoEmoji: "🛒", shopAddress: "",
...
export function OnboardingModule({ packages, addToast }: Props) {
  ...
  if (launched && credentials) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        <SectionHeader title="🚀 Onboard New User" subtitle="Step-by-step guided shop setup" />
        <Card style={{ textAlign: "center", padding: "40px 24px" }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
            <Btn variant="primary" onClick={() => window.open(credentials.shopUrl, "_blank")}>↗ Visit Shop</Btn>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <SectionHeader title="🚀 Onboard New User" subtitle="Register a user, configure their shop, and get them live end-to-end" />
      ...
      <Card>
        ...
        {step === 1 && (
          <div>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: T.textMuted, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>Shop Logo Emoji</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {EMOJI_OPTIONS.map(em => (
                  <button key={em} onClick={() => set("shopLogoEmoji", em)} style={{
                    width: 42, height: 42, borderRadius: 8, border: `2px solid ${form.shopLogoEmoji === em ? T.primary : T.border}`,
                    background: form.shopLogoEmoji === em ? T.primaryLight : T.surface,
            ...
            {form.shopName && (
              <div style={{ marginTop: 16, background: T.bg, borderRadius: T.radiusCard, padding: "14px 18px", display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: 10, background: T.primaryLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>{form.shopLogoEmoji}</div>
                <div>
          ...
          <div>
            ...
            {products.map((p, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: T.bg, borderRadius: T.radius, marginBottom: 8 }}>
                ...
                <button onClick={() => setProducts(prev => prev.filter((_, ii) => ii !== i))} style={{ background: "none", border: "none", cursor: "pointer", color: T.danger, fontSize: 16 }}>✕</button>
              </div>
            ))}
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
              ...
              <ReviewSection title="🏪 Shop">
                <ReviewRow label="Name" value={`${form.shopLogoEmoji} ${form.shopName}`} />
              ...
              <ReviewSection title="🛍️ Products">
            ...
            <div style={{ background: T.successLight, border: `1px solid ${T.success}`, borderRadius: T.radiusCard, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "#166534" }}>
              ✅ Everything looks good! Clicking "Launch Shop" will create the user account, configure the shop, apply the package, and go live.
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 20, paddingTop: 16, borderTop: `1px solid ${T.border}` }}>
          ...
          {step < STEPS.length - 1
            ...
            : <Btn variant="success" onClick={handleLaunch} disabled={launching}>{launching ? "Launching…" : "🚀 Launch Shop"}</Btn>
          }
        </div>
      </Card>
    </div>
...
function ErrMsg({ msg }: { msg: string }) {
  return <div style={{ fontSize: 11, color: T.danger, marginTop: -10, marginBottom: 10 }}>⚠ {msg}</div>;
}
function ReviewSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ background: T.bg, borderRadius: T.radiusCard, padding: "12px 16px" }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: T.textMuted, marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.05em" }}>{title}</div>
      {children}
    </div>
  );
}
