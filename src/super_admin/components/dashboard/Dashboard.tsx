...
import {
  ...
  ShoppingBag, Eye, Headphones, AlertTriangle, ArrowUpRight,
  TrendingUp, User, Rocket, Megaphone, Globe, Monitor,
} from "lucide-react";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: "#fff", display: "flex", alignItems: "center", gap: 8 }}>Welcome back, {admin.name.split(" ")[0]} <Hand size={18} /></div>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
            {admin.privilegeLevel === "full" ? <Sparkles size={12} color="#fff" /> : <CircleDot size={12} color="#fff" />}
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12 }}>
          <StatCard icon={Store} label="Total Shops" value={metrics.totalShops} sub={`${metrics.activeShops} active`} color={T.primary} />
          <StatCard icon={Users} label="Subscribers" value={metrics.totalSubscribers} sub="registered users" color={T.accent} />
          <StatCard icon={Wallet} label="Total Revenue" value={fmt(metrics.totalRevenue)} sub={`${fmt(metrics.monthlyRevenue)} this month`} color={T.success} />
          <StatCard icon={Package} label="Total Orders" value={metrics.totalOrders.toLocaleString()} sub="all shops" />
          <StatCard icon={ShoppingBag} label="Abandoned Carts" value={metrics.abandonedCarts} sub="unpurchased carts" color={T.warning} />
          <StatCard icon={Eye} label="Page Views" value={metrics.totalPageViews.toLocaleString()} sub="total visits" color={T.primary} />
          <StatCard icon={Headphones} label="Open Tickets" value={metrics.activeTickets} sub={`Avg ${metrics.avgResponseTime}h response`} color={metrics.activeTickets > 0 ? T.danger : T.success} />
          <StatCard icon={AlertTriangle} label="Suspended" value={metrics.suspendedShops} sub="shops suspended" color={T.danger} />
        </div>
      </div>
              ...
              return (
                <div key={s.id}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 3 }}>
                    <span style={{ color: T.text, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6 }}>
                      {(() => { const LogoIcon = getLogoIcon(s.logoIcon); return <LogoIcon size={13} />; })()} {s.name}
                    </span>
      ...
      <Card>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          ...
          <Btn variant="ghost" size="sm" onClick={() => setShowShopVisits(v => !v)}>
            {showShopVisits ? "Show Platform Traffic" : <>Per-Shop Visits <ArrowUpRight size={14} /></>}
          </Btn>
        </div>
        ...
        {showShopVisits ? (
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 20, marginTop: 12, fontSize: 12, color: T.textMuted }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}><TrendingUp size={14} /> Total views: <strong style={{ color: T.text }}>{metrics.dailyVisits.reduce((a, b) => a + b.visits, 0).toLocaleString()}</strong></span>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}><User size={14} /> Unique visitors: <strong style={{ color: T.text }}>{metrics.dailyVisits.reduce((a, b) => a + b.uniqueVisitors, 0).toLocaleString()}</strong></span>
            </div>
          </div>
        )}
      </Card>
      ...
      {expiringShops.length > 0 && (
        <Card style={{ borderLeft: `4px solid ${T.warning}` }}>
          <h4 style={{ margin: "0 0 10px", fontSize: 13, fontWeight: 700, color: T.warning, display: "flex", alignItems: "center", gap: 6 }}><AlertTriangle size={14} /> Subscriptions Expiring Soon</h4>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {expiringShops.map(s => {
              const days = Math.ceil((new Date(s.expiresAt).getTime() - Date.now()) / 86400000);
              return (
                <div key={s.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 13 }}>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                    {(() => { const LogoIcon = getLogoIcon(s.logoIcon); return <LogoIcon size={14} />; })()} <strong>{s.name}</strong> — {s.ownerName}
                  </span>
      ...
      <Card>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
          <Btn onClick={() => onNavigate("onboarding")} variant="primary"><Rocket size={14} /> Onboard New User</Btn>
          <Btn onClick={() => onNavigate("issues")} variant="ghost"><Headphones size={14} /> View Support Tickets</Btn>
          <Btn onClick={() => onNavigate("reminders")} variant="ghost"><Megaphone size={14} /> Send Reminder</Btn>
          <Btn onClick={() => onNavigate("shops")} variant="ghost"><Store size={14} /> Manage Shops</Btn>
          <Btn onClick={() => onNavigate("marketplace")} variant="ghost"><Globe size={14} /> Marketplace</Btn>
          {admin.privilegeLevel === "full" && (
            <Btn onClick={() => onNavigate("dbterminal")} variant="accent"><Monitor size={14} /> DB Terminal</Btn>
          )}
        </div>
      </Card>
    </div>
